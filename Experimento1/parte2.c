/*
Alexandre Gil Lujan 			17148826
Gabriel Albertin dos Santos 	17214172
Gabriela Nelsina Vicente 		17757089
Matheus Valbert 				17055641
Tomas Martinolli Verwiebe 		17108978
*/

/*******************************************************************************
* Este programa esta baseado no segundo experimento do curso sobre tempo real 
* do Laboratorio Embry-Riddle
* 
* Seguem os comentarios originais:
*
* Experiment #2: Multi-Tasking, Measuring Drift
*
*    Programmer: Eric Sorton
*          Date: 1/27/97
*           For: MSE599, Special Topics Class
*
*       Purpose: When a basic sleep call is used to determine the precise time
*                when an action will occur the problem of drift occurs. 
*                The measurement of time is imprecise.  Similarly, the time in 
*                which the sleep call returns is imprecise.  Over time, this 
*                will cause the ocurrence of time to drift.  Just as if a clock 
*                loses 1 second every day, over one day, it is significant, but 
*                over a year, it loses 365 seconds, which is over 6 minutes.  
*                This is an example of drift.
*
*       Proposito: Quando uma chamada básica sleep e usada para determinar o
*                instante exato em que alguma acao vai ocorrer, ocorre o problema
*                do desvio. A medicao de tempo e imprecisa. Similarmente, o tempo
*                que demora o retorno da chamada sleep tambem e impreciso. Ao
*                longo do tempo, isto ocasionara um desvio de tempo. Algo como se
*                um relogio perdesse um segundo a cada dia. Ao longo de um dia, 
*                essa diferenca e insignificante, mas, ao longo de um ano, sao 
*                perdidos 365 segundos, o que e superior a 6 minutos. Este e um
*                exemplo de desvio.
*
*******************************************************************************/

/*
 * Includes Necessarios, verifique se as bibliotecas no diretorio sys/ estao
 * lah. Caso nao estejam, verifique onde estao e altere o include
 */

#include <stdlib.h>
#include <sys/time.h>		/* for gettimeofday() */
#include <unistd.h>		/* for gettimeofday() and fork() */
#include <stdio.h>		/* for printf() */
#include <sys/types.h>		/* for wait() */
#include <sys/wait.h>		/* for wait() */
#include <signal.h>
/*
 * Pergunta 1: o que o compilador gcc faz com o arquivo .h, cujo nome aparece após o include?
 * 
 * Os arquivos .h, sao bibliotecas de comando, quando voce digita um comando, ele procura nas bibliotecas incluidas o comando que no caso seria como um atalho para aquela lista de comandos.
 */

/*
 * Pergunta 2: apresentar (parcialmente) e explicar o que há em <stdio.h>
 * 
 * Basicamente, a biblioteca stdio, tem todos os comandos padrao de entrada e saida de dados (standard in/out header)
 */

/*
 * Pergunta 3: qual eh a funcao da diretiva include (linha que começa com #), com relação ao compilador?
 * 
 * Este comando serve para o fazer uma copia de tudo que tem na biblioteca selecionada e passar para o seu programa, para voce poder usar comandos com facilidade
 */

/*
 * NO_OF_ITERATIONS e o numero de vezes que vai se repetir o loop existente
 * em cada futuro processo filho. 
 */

#define NO_OF_ITERATIONS	1000

/*
 * NO_OF_CHILDREN eh o numero de filhos a serem criados, cada qual responsavel
 * pela medida do desvio. 
 */

#define NO_OF_CHILDREN	3

/*
 * SLEEP_TIME corresponde a qwnloads$ Filho #1 -- desvio total: 0.121 -- desvio medio: 0.0
Filho #2 -- desvio total: 0.121 -- desvio medio: 0.0
Filho #3 -- desvio total: 0.121 -- desvio medio: 0.0
filho
uantidade de tempo para ficar bloqueado.
 */

#define SLEEP_TIME 400
/*
 * MICRO_PER_SECOND define o numero de microsegundos em um segundo
 */

#define MICRO_PER_SECOND	1000000

/*
 * Programa Principal. Contem tanto o codigo para o processo pai como
 * o codigo dos futuros filhos
 */

void sighandler() {

   exit(1);
}

int main( int argc, char *argv[] )
{
/*
 * Pergunta 4: o que são e para que servem argc e argv? Não esqueca de
 * considerar o * antes de argv.
 * 
 * O argc serve para contar a quantidade de argumentos e o argv guarda o comando digitado em um ponteiro para um matriz de string
 */
	/*
	 * start_time e stop_time conterao o valor de tempo antes e depois
	 * que as trocas de contexto comecem
         */

	struct timeval start_time;
	struct timeval stop_time;

      /*
       * Outras variaveis importantes
       */

      float drift;
      int count;
      int child_no;

	/*
	 * Criacao dos processos filhos
	 */
	int rtn;
	pid_t rtnSave[3];
	for( count = 0; count < NO_OF_CHILDREN; count++ ) {
		rtn = fork();
		rtnSave[count] = rtn;
		if(rtn == 0)
			break;
	}



	/*
	 * Verifica-se rtn para determinar se o processo eh pai ou filho
	 */

	if( rtn == 0 ) {

	/*
         * Portanto, sou filho. Faco coisas de filho. 
         */
		char numero_child[10];
		sprintf(numero_child,"%d",count);
		char numero_interacao[10];
		sprintf(numero_interacao,"%d",NO_OF_ITERATIONS);
		char numero_sleep[10];
		sprintf(numero_sleep,"%d",SLEEP_TIME);
		char numero_micro[10];
		sprintf(numero_micro,"%d",MICRO_PER_SECOND);
	
		char * argv_list[] = {numero_child,numero_interacao,numero_sleep,numero_micro, NULL}; 
  
	      // the execv() only return if error occured. 
	      // The return value is -1 
	      execv("./exec",argv_list);  

		/*
		 * Primeiro, obtenho o tempo inicial.
		 */

	} else {
		/*
		 * Sou pai, aguardo o termino dos filhos
		 */
		for( count = 0; count < NO_OF_CHILDREN; count++ ) {
			//wait(NULL);
			signal(SIGCHLD, sighandler);
			pause();
			kill(rtnSave[count], SIGCHLD);
		}
	}

	exit(0);
}



/*

Pergunta 1:
gcc Experimento1.c -o experimento1

Pergunta 2:
	O efeito da diretiva "&" faz com que os processos executados sejam utilizados da mesma forma que um processo utilizado pelo comando fork. Além do programa ser realizado em background(paralelamente). 

Pergunta 3:
	O motivo do uso do "./" é para indicar que o arquivo a ser executado está no diretório atual.char *args[]={"./EXEC",NULL}; 
        execvp(args[0],args); 
	O gcc não utiliza /., pois ele já é obrigatoriamente executado no diretório atual devido ao fato de ter de sinalizar qual arquivo em formato c será transformado em um arquivo executável com o nome indicado ou com o nome "a.out" caso nenhum nome para o arquivo seja indiciado.

Pergunta 4:
	7.7GB de RAM
	Intel® Core™ i5-4570 CPU @ 3.20GHz × 4

Entre pergunta 4 e fim: alteração de duas variaveis

	Primeira alteração: modificando a variável NO_OF_ITERATIONS de 1000 para 3000.
Desvio geral aumentado em 3x (filho 1 = 0.367, filho 2 = 0.367, filho 3 = 0.368 -- Desvio Medio = 0.0001)
	Normalmente desvio geral(filho 1,2 e 3 = 0.114 e desvio medio 0,0001)

	Segunda alteração: modificando sleep time de 1000 para 5000
	
	Não houve alterações no desvio total e no desvio medio observados.
	
*/
