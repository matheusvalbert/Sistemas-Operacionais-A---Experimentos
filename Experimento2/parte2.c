 ...
 * Filhos
 */
void Receiver(int queue_id, int num, key_t key);
void Sender(int queue_id, int num);
void ResultReceiver(key_t key);
/*
...
int main( int argc, char *argv[] )
{
    ...
    */
    int queue_id;
    key_t key = MESSAGE_QUEUE_ID, keyCh = MSG_KEY;

	char numerostring[20];
	scanf("%s", numerostring);
	num = atoi(numerostring);
	num = num * 512;
	...
	*/
	if( rtn == 0 && count == 1 ) {
		...
		*/
        printf("Receptor 2 iniciado ...\n");
        ResultReceiver(keyCh);
        exit(0);
	}
...
//struct para a segunda fila de mensagens
typedef struct {
...
void Receiver(int queue_id, int num, key_t key)
{
	...
	if((queue_id_child = msgget(key, IPC_CREAT | 0666)) == -1 ) {
		fprintf(stderr,"Impossivel criar a fila de mensagens!\n");
		exit(1);
	}
	...
	*/
	...
	for( count = 0; count < NO_OF_ITERATIONS; count++ ) {
		 ...
		 */
		if( msgrcv(queue_id,(struct msgbuf *)&message_buffer,num,MESSAGE_MTYPE,0) == -1 ) {
	...
	*/
	...
	if(msgsnd(queue_id_child,(struct msgbuf*)&message,sizeof(bufmsg),0) == -1 ) {
	...
	message.mtype = MESSAGE_MTYPE;
	message.mfloat = (total/NO_OF_ITERATIONS);
	...
	//printf( "O tempo medio de transferencia: %lf\n", total / num );
	//fprintf(stdout, "O tempo maximo de transferencia: %lf\n", max );
...
void Sender(int queue_id, int num)
{
	...
	for( count = 0; count < NO_OF_ITERATIONS; count++ ) {
		 ...
		 */
		if( msgsnd(queue_id,(struct msgbuf *)&message_buffer,num,0) == -1 ) {
			fprintf(stderr, "Impossivel enviar mensagem!\n");
			exit(1);
		}
...
void ResultReceiver(key_t key) {
    ...
    for( count = 0; count < 4; count++ ) {
		 ...
		 * Recebe qualquer mensagem do tipo MESSAGE_MTYPE
		 */
		if(msgrcv(queue_id_child,(struct msgbuf *)&message,sizeof(bufmsg),MESSAGE_MTYPE,0) == -1 ) {
			fprintf(stderr, "Impossivel receber mensagem!\n");
			exit(1);
		}
		...
		/*
        * Removendo a fila 2 de mensagens
        */
        if( msgctl(queue_id_child,IPC_RMID,NULL) == -1 ) {
			fprintf(stderr,"Impossivel remover a 2 fila!\n");
			exit(1);
		}
